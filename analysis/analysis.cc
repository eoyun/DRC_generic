...
int main(int argc, char* argv[]) {
  ...
  TH1F* tEdep = new TH1F("totEdep","Total Energy deposit;MeV;Evt",100,0.,200);
 ...
 // tHit_C->Sumw2(); tHit_C->SetLineColor(kBlue); tHit_C->SetLineWidth(2);
  ...
  TH1F* recoE = new TH1F("reco_E","Reco Energy;MeV;Evt",100,0,200);
 ...
 // tNhit_C->Sumw2(); tNhit_C->SetLineColor(kBlue); tNhit_C->SetLineWidth(2);
  ...
  while (drInterface->numEvt() < entries) {
    ...
    float Edep = 0.;
    int nHitCry = 0;
    for (auto stepItr = drEvt.steps.begin();stepItr != drEvt.steps.end();++stepItr){
      auto step = *stepItr;
      if (step.vy>1) Edep += step.Edep;
    }
    tEdep->Fill(Edep);
