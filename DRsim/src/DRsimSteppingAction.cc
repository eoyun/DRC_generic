...
void DRsimSteppingAction::UserSteppingAction(const G4Step* step) {
  ...
  fEdep.EdepModule = (step->GetTrack()->GetPosition()[2]>0) ? fEdep.Edep : 0;
   
  if ( fEdep.Edep > 0. ) {
    fEventAction->fillEdeps(fEdep);
    G4cout<<"###### this is debug line ######"<<G4endl;
    G4cout<<"total energy deposit is "<<fEdep.EdepModule<<G4endl;
    G4cout<<"deposit position is "<<step->GetTrack()->GetPosition()[2]<<G4endl;
    G4cout<<"################################"<<G4endl;
  }
